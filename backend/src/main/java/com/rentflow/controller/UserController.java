...
public class UserController {
    ...
    public ResponseEntity<?> updateProfile(
        ...
        try {
            User updated = userService.updateProfile(userDetails.user().getId(), request);
    ...
    public ResponseEntity<?> changePassword(
        ...
        try {
            userService.changePassword(userDetails.user().getId(), request);
    ...
    public ResponseEntity<?> verifyPhone(
        ...
        try {
            User updated = userService.verifyPhone(userDetails.user().getId(), request.getPhone());
            return ResponseEntity.ok(UserResponse.fromEntity(updated));
        } catch (RuntimeException e) {
            log.error("verifyPhone failed", e);
            return ResponseEntity.badRequest().body(Map.of("message", e.getMessage()));
        }
    }
}
