...
public class RentalController {
    ...
    public ResponseEntity<List<TenantHistoryResponse>> getLandlordHistory(
            @AuthenticationPrincipal CustomUserDetails userDetails) {
        List<TenantHistory> history = tenantHistoryService.getHistoryByTenant(userDetails.user().getId());
    ...
    public ResponseEntity<?> leaveRental(
            ...
            @AuthenticationPrincipal CustomUserDetails userDetails) {
        try {
            Property property = propertyService.leaveRental(propertyId, userDetails.user().getId());
            return ResponseEntity.ok(PropertyResponse.fromEntity(property));
        } catch (RuntimeException e) {
            log.error("leaveRental failed: propertyId={}", propertyId, e);
